...
def read_gsc_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mapping = {
        ...
        'Top pages': 'page',  'Page': 'page',  'Pages': 'page',
        'URL': 'page', 'Landing Page': 'page',
        'Clicks': 'clicks', 'Url Clicks': 'clicks',
        'Impressions': 'impressions',
        'CTR': 'ctr', 'URL CTR': 'ctr', 'CTR (%)': 'ctr',
    ...
    if 'ctr' in df.columns:
        if df['ctr'].dtype == object:
            df['ctr'] = df['ctr'].astype(str).str.rstrip('%').astype(float, errors='ignore')
...
def build_query_summary(df: pd.DataFrame) -> pd.DataFrame:
    """One-row-per-query grouped view."""
    rows = []
    for q, grp in df.groupby('query'):
        ...
        rows.append({
            ...
            'Total Clicks':      grp['clicks'].sum(),
            'Total Impressions': grp['impressions'].sum(),
            'Best Position':     grp['position'].min(),
            'Worst Position':    grp['position'].max(),
            'Position Spread':   round(grp['position'].max() - grp['position'].min(), 1),
            'Best URL Slug':     best['slug'],
            'All URL Slugs':     ' | '.join(grp.sort_values('impressions', ascending=False)['slug'].tolist()),
        })
    out = pd.DataFrame(rows).sort_values('Total Impressions', ascending=False)
    return out
...
query_sum['_sev'] = query_sum.apply(
    lambda r: severity(r['Best Position'], r['Total Impressions']), axis=1)
...
with tab1:
    ...
    display_qs.insert(2, 'Severity',
        display_qs.apply(lambda r: severity(r['Best Position'], r['Total Impressions']), axis=1))

    if not show_full_urls:
        display_qs['Best URL Slug'] = display_qs['Best URL Slug'].str[:60]
        display_qs['All URL Slugs'] = display_qs['All URL Slugs'].str[:120]
    ...
    with dl2:
        st.download_button("📥 Download Excel",
            data=to_excel({
                ...
                'Detail View': cannibs.rename(columns={'slug': 'URL Slug'}),
            }),
...
with tab2:
    ...
    detail_display.insert(2, 'Severity',
        detail_display.apply(lambda r: severity(r['Avg Position'], r['Impressions']), axis=1))

    if not show_full_urls:
        detail_display['URL Slug'] = detail_display['URL Slug'].str[:70]
        ...
        for q in high_queries[:30]:
            ...
            with st.expander(
                ...
                disp = qdata.rename(columns={
                    'slug': 'URL Slug', 'clicks': 'Clicks',
                    'impressions': 'Impressions', 'ctr': 'CTR (%)',
                    'position': 'Avg Position', 'competing_pages': 'Competing Pages',
                })[['URL Slug', 'Clicks', 'Impressions', 'CTR (%)', 'Avg Position', 'Competing Pages']]
...
with tab4:
    ...
    priority_df = query_sum[['Query', 'Competing Pages', 'Total Impressions',
                              'Total Clicks', 'Best Position', '_sev']]\
                      .rename(columns={'_sev': 'Severity'})\
                      .sort_values('Total Impressions', ascending=False)\
                      .head(50)
